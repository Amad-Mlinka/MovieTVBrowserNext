...
export const getStaticPaths = async () => {
    ...
    const paths1 = movies.results.map(movie => {
        return {
            params: { id: movie.id.toString() }
        }
    })

    const paths2 = recommendedMovies.results.map(movie => {
        return {
            params: { id: movie.id.toString() }
        }
    })
    const paths3 = discoverMovies.results.map(movie => {
        return {
            params: { id: movie.id.toString() }
        }
    })
    const paths4 = popularMovies.results.map(movie => {
        return {
            params: { id: movie.id.toString() }
        }
    })
