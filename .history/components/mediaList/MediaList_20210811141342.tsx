...
//mediaType="movie" media={props.movies.results} heading="Movies" subHeading="Discover new Movies picked just for you" 

const MediaList = ({ mediaType, media, heading, subHeading }: mediaListInterface) => {
    console.log(typeof  keys<mediaListInterface>())
            ...
            <div className={mediaListStyles.mediaList} >
                <div className={mediaListStyles.content}>
                    ...
                    </div>
                    {
                        ...
                        {media.map((media) => (
                            <Media key={media.id} className={mediaListStyles.mediaItem} id={media.id} image={media.poster_path} rating={Math.round((media.vote_average / 2) * 10) / 10} title={mediaType == "movie" ? media.title : media.name} year={props.mediaType === "movie" ? media.release_date : media.first_air_date} />
                        ))}
