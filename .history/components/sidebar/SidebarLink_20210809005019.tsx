...
const SidebarLink = (props) => {
    ...
    return (
        <>
            {
                props.type === types[0] ?
                    <li className={`${sidebarStyles.sidebarLink} ${sidebarStyles.sidebarSearch} ${props.sidebarOpen ? sidebarStyles.sidebarSearchActive : ""}`} onClick={()=>{
                    ...
                    :
                    props.type === types[1] ?
                        <li className={sidebarStyles.sidebarLink} onClick={()=>{
                            if(!props.sidebarOpen)  props.setSidebarOpen(!props.sidebarOpen)
                        }}>
                        ...
                        :
                        <li className={`${sidebarStyles.sidebarLink}  ${props.sidebarOpen ? sidebarStyles.sidebarLinkOpen : ""}`} onClick={() => {
                            if(!props.sidebarOpen)  props.setSidebarOpen(!props.sidebarOpen)
                            setOpen(!open)
                        }
                        }>
